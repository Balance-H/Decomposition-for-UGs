...
file_list <- c(
  "Animal-Network.txt","bio-CE-GT.txt","DD6.txt", "as20000102.txt", "CA-HepTh.txt"
)

# Load RData files from the data subfolder in the current working directory
load("./data/BRCA.RData")  # load gene data
gene  <- brca_data[, 1:10000]
n_obs <- nrow(gene)
Sp <- cov2cor(cov(gene))
...
rownames(design) <- with(
  design,
  interaction(rep, approach, method, marg, dat, bench, network, sep = "_")
)
...
train_network <- function(experiment_row, folder_path, global_args) {
  ...
  if (experiment_row$dat == "n01") {
    ...
    da <- Sp  # Sp is assumed to be a matrix defined earlier
  }
  
  da <- da[seq_len(n_nodes), seq_len(n_nodes)]  # n×n matrix
  ...
  if (!is.null(experiment_row$approach) && tolower(experiment_row$approach) == "direct") {
    ...
    cat(sprintf("[%s] network=%s, approach=%s, method=%s, dat=%s, time=%.2f secs\n",
                format(Sys.time(), "%Y-%m-%d %H:%M:%S"),
                experiment_row$network,
  ...
  make_arg_list <- function(edges_list, dat_matrix, type_label) {
    lapply(edges_list, function(sub) {
      S <- dat_matrix[sub$nodes_global, sub$nodes_global, drop = FALSE]
  ...
  atom_arg_lst <- make_arg_list(atom_edges_list, da, "atom")
  ...
  for(nodes in small_atoms) {
    if(length(nodes) == 1) {
      Q[nodes, nodes] <- Q[nodes, nodes] + 1 / (da[nodes, nodes] + 1e-8)
    } else {
      S_sub <- as.matrix(da[nodes, nodes])
  ...
  for(nodes in separators) {
    S_sub <- as.matrix(da[nodes, nodes, drop = FALSE])
    storage.mode(S_sub) <- "double"
    
    K_sub <- solve(S_sub + 1e-8 * diag(ncol(S_sub)))
...
result <- df %>%
  group_by(network, approach, method, dat) %>%
  summarise(
    mean_time  = round(mean(time, na.rm = TRUE), 4),
    sd_time    = round(sd(time, na.rm = TRUE), 4),  
    n_reps     = n(),
    .groups    = 'drop'
  ) %>%
  arrange(network, method, dat, approach)

print(result)

html_code <- htmlTable(result, caption = "Summary of network average compute time")
write(html_code, file = "result_time.html")
