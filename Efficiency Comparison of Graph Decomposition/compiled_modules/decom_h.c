...
/**
 * igraph_CMSA - 基于节点集 r_nodes 对图 graph 进行 CMSA（Clique Minimal Separator Algorithm）凸包扩展，
 *              并输出扩展后的节点集 H_out。
 *
 * 输入参数：
                ...
                for (igraph_integer_t j = 0; j < bsize && !found; j++) {
                    ...
                    for (igraph_integer_t k = j + 1; k < bsize && !found; k++) {
                        ...
                        if (!are_adj) {
                            ...
                            if (strcmp(method, "cmsa") == 0) {
                                // cmsa方法：调用两次close_separator
                            ...
                            } else if (strcmp(method, "ipa") == 0) {
                                // ipa方法：计算所有最短路径上的节点集合

                                igraph_vector_int_list_t shortest_paths;  // 存储所有路径的顶点列表
                                igraph_vector_int_list_init(&shortest_paths, 0);

                                // edges 和 nrgeo 不需要，传 NULL
                                igraph_vector_int_list_t *edges = NULL;
                                igraph_vector_int_t *nrgeo = NULL;

                                // 构造目标节点集合（单个目标节点 b）
                                igraph_vs_t vs;
                                igraph_vs_1(&vs, b);

                                // 调用 igraph_get_all_shortest_paths
                                igraph_get_all_shortest_paths(&subgraph, &shortest_paths, edges, nrgeo, a, vs, IGRAPH_ALL);

                                igraph_vs_destroy(&vs);

                                // 标记访问过的节点，避免重复加入
                                igraph_vector_bool_t visited;
                                igraph_vector_bool_init(&visited, igraph_vcount(&subgraph));

                                // 遍历所有路径，提取路径节点并映射回全局索引加入 H_local
                                igraph_integer_t n_paths = igraph_vector_int_list_size(&shortest_paths);
                                for (igraph_integer_t i = 0; i < n_paths; ++i) {
                                    igraph_vector_int_t *path = igraph_vector_int_list_get_ptr(&shortest_paths, i);
                                    for (igraph_integer_t j = 0; j < igraph_vector_int_size(path); ++j) {
                                            igraph_integer_t idx = VECTOR(*path)[j];
                                        if (!VECTOR(visited)[idx]) {
                                            igraph_vector_int_push_back(&H_local, VECTOR(invmap)[idx]);
                                            VECTOR(visited)[idx] = 1;
                                        }
                                        
                                    }
                                }
                                igraph_vector_bool_destroy(&visited);
                                igraph_vector_int_list_destroy(&shortest_paths);
                                igraph_destroy(&subgraph);
                                igraph_vector_int_destroy(&map);
                                igraph_vector_int_destroy(&invmap);
                            }
...
/*
 * 递归图分解函数，基于最大基数搜索（MCS）序列，结合局部子图和映射关系进行迭代分解。
 * 参数:
 *  - g: 输入图的常量指针
 *  - method: 使用的求凸包方法，如"cmsa"或"ipa"
 *  - atoms: 存储分解块的向量指针，元素为全局节点编号向量
 *  - separators: 存储分离子块的向量指针，元素为全局节点编号向量
 * 返回:
...
igraph_error_t recursive_decom(
    ...
    igraph_vector_ptr_push_back(&A_list, Vfull);
    ...
    while (!igraph_vector_ptr_empty(&A_list)) {
        igraph_vector_int_t *A_global = (igraph_vector_int_t*)VECTOR(A_list)[igraph_vector_ptr_size(&A_list) - 1];
        ...
        if (is_clique) {
            ...
            find_convex_hull(&sub_g, &N_v, &H, method);
        }
        ...
        for (igraph_integer_t i = 0; i < ncomps; i++) {
            ...
            igraph_vector_ptr_push_back(&A_list, A_global_ptr);
